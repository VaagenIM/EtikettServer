...
def _qr_label(content: InventoryItem) -> PIL.Image:
    ...
    font_id = ImageFont.truetype(FONT_MONO, size=int(ly / 2.2))
    font_name = ImageFont.truetype(FONT_NAME, size=int(ly / 5.5))
    ...
    logo_draw.rounded_rectangle(
        (logo_start, 2, logo_end, logo_height),
        fill="black",
        outline="black",
        width=15,
        radius=25,
    ...
    while (
        info_draw.textbbox((0, 0), content.id, font=font_id, align="left", anchor="mm")[
            2
        ]
        * 2
    ) > lx - qr_size[0] - 9:
    ...
    while (
        info_draw.textbbox(
            (0, 0), content.item_name, font=font_name, align="left", anchor="mm"
        )[2]
        * 2
    ) > lx - qr_size[0] - 9:
    ...
    # Draw the text
    draw_text(content.id, ly / 5 - 5, font_id, stroke_width=1)
    draw_text(content.item_name, ly / 2 - 10, font_name)
...
def _barcode_label(content: InventoryItem) -> PIL.Image:
    ...
    image = barcode.get(
        "code128", content.id, writer=barcode.writer.ImageWriter()
    ).render(options)
...
def paste_on_label(label: PIL.Image) -> PIL.Image:
    ...
    new_label.paste(
        label,
        (
            int((output_label_dimensions[0] - label.width) / 2),
            int((output_label_dimensions[1] - label.height) / 2),
        ),
    )
...
def create_label(
    content: InventoryItem, variant: LabelType = LabelType.QR
) -> PIL.Image:
